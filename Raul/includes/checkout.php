
<span class="b p"><!--  --></span>
<div class="wrapper-holder">
    <div class="wrapper">
    <div class="b">
		<div class="b contentBox wrapper">
			<span class="b p5"><!--  --></span>
			<div class="headerTituloRed b">
				COD#P543
			</div>
			<div class="b novidadesHeaderFooter">
				<div class="headerSubtitulo b l">
					Camisa Lorem Ipsum 
				</div>
				<div class="headerMenu b r">
					<h1>Por: Raul Santana</h1>
					<div class="c"><!--  --></div>
				</div>
				<div class="c"><!--  --></div>
			</div>
			<div class="c"><!--  --></div>
			<span class="b p5"><!--  --></span>
			<div class="b p checkoutRow">
				<span class="b l ">
					<span class="b">
						<img class="prodPic" src="img/produtos/produto/produto01.gif"/>
					</span>
					<span class="b p5"><!--  --></span>
                    <span class="b">
						<span class="b l">
							
							<span class="b l sprite star"><!--  --></span>
							<span class="b l p5">349 Votos</span>
							<div class="c"><!--  --></div>
							
						</span>
						<div class="c"><!--  --></div>
					</span>
				</span>
				<span class="b l p5"><!--  --></span>
				<span class="b l checkoutInfo">
					<span class="b"><h1>Camis Lorem Ipsum por Raul Santana</h1></span>
					<span class="b"><p>Consectetur adipiscing elit. Praesent neque orci, consectetur id faucibus eget, ullamcorper ut enim. Aliquam sed leo mattis tortor vulputate sagittis. Aenean nisl velit, fermentum non faucibus nec, imperdiet hendrerit enim. Nam non diam ligula. Aenean quis dignissim sem.</p></span>
					<span class="b p5"><!--  --></span>
					<span class="b">
							<span class="b">
								<span class="b"><h1>Quantidade</h1></span>
								<span class="b p5"><!--  --></span>
								<span class="b">
							<ul class="inline tamanChoose">
								<li>Tamanho Escolhido:</li>
								<li><a href="#">PP</a></li>
								<li>Quantidade/Un.:</li>
								<li><select><option>01</option></select></li>
							</ul>
							</span>
						</span>
					</span>
				</span>
				<div class="c"><!--  --></div>
					<span class="b p5"><!--  --></span>
					<span class="b">
						
						<span class="b r p5"><!--  --></span>
						<span class="b r"><a href="#"><span class="b removerProduto">Remover Produto &#9654;</span></a></span>
						
						<div class="c"><!--  --></div>
					</span>
			</div>
		</div>
		<div class="b wrapper contentBottom">
			<img src="img/content-bottom.gif">
		</div>
		</div>
		
    <div class="b">
		<div class="b contentBox wrapper">
			<span class="b p5"><!--  --></span>
			<div class="headerTituloRed b">
				COD#P543
			</div>
			<div class="b novidadesHeaderFooter">
				<div class="headerSubtitulo b l">
					Camisa Lorem Ipsum 
				</div>
				<div class="headerMenu b r">
					<h1>Por: Raul Santana</h1>
					<div class="c"><!--  --></div>
				</div>
				<div class="c"><!--  --></div>
			</div>
			<div class="c"><!--  --></div>
			<span class="b p5"><!--  --></span>
			<div class="b p checkoutRow">
				<span class="b l ">
					<span class="b">
						<img class="prodPic" src="img/produtos/produto/produto01.gif"/>
					</span>
					<span class="b p5"><!--  --></span>
                    <span class="b">
						<span class="b l">
							
							<span class="b l sprite star"><!--  --></span>
							<span class="b l p5">349 Votos</span>
							<div class="c"><!--  --></div>
							
						</span>
						<div class="c"><!--  --></div>
					</span>
				</span>
				<span class="b l p5"><!--  --></span>
				<span class="b l checkoutInfo">
					<span class="b"><h1>Camis Lorem Ipsum por Raul Santana</h1></span>
					<span class="b"><p>Consectetur adipiscing elit. Praesent neque orci, consectetur id faucibus eget, ullamcorper ut enim. Aliquam sed leo mattis tortor vulputate sagittis. Aenean nisl velit, fermentum non faucibus nec, imperdiet hendrerit enim. Nam non diam ligula. Aenean quis dignissim sem.</p></span>
					<span class="b p5"><!--  --></span>
					<span class="b">
							<span class="b">
								<span class="b"><h1>Quantidade</h1></span>
								<span class="b p5"><!--  --></span>
								<span class="b">
							<ul class="inline tamanChoose">
								<li>Tamanho Escolhido:</li>
								<li><a href="#">PP</a></li>
								<li>Quantidade/Un.:</li>
								<li><select><option>01</option></select></li>
							</ul>
							</span>
						</span>
					</span>
				</span>
				<div class="c"><!--  --></div>
					<span class="b p5"><!--  --></span>
					<span class="b">
						
						<span class="b r p5"><!--  --></span>
						<span class="b r"><a href="#"><span class="b removerProduto">Remover Produto &#9654;</span></a></span>
						
						<div class="c"><!--  --></div>
					</span>
			</div>
		</div>
		<div class="b wrapper contentBottom">
			<img src="img/content-bottom.gif">
		</div>
		</div>
		
    <div class="b">
		<div class="b contentBox wrapper">
			<span class="b p5"><!--  --></span>
			<div class="headerTituloRed b">
				COD#P543
			</div>
			<div class="b novidadesHeaderFooter">
				<div class="headerSubtitulo b l">
					Camisa Lorem Ipsum 
				</div>
				<div class="headerMenu b r">
					<h1>Por: Raul Santana</h1>
					<div class="c"><!--  --></div>
				</div>
				<div class="c"><!--  --></div>
			</div>
			<div class="c"><!--  --></div>
			<span class="b p5"><!--  --></span>
			<div class="b p checkoutRow">
				<span class="b l ">
					<span class="b">
						<img class="prodPic" src="img/produtos/produto/produto01.gif"/>
					</span>
					<span class="b p5"><!--  --></span>
                    <span class="b">
						<span class="b l">
							
							<span class="b l sprite star"><!--  --></span>
							<span class="b l p5">349 Votos</span>
							<div class="c"><!--  --></div>
							
						</span>
						<div class="c"><!--  --></div>
					</span>
				</span>
				<span class="b l p5"><!--  --></span>
				<span class="b l checkoutInfo">
					<span class="b"><h1>Camis Lorem Ipsum por Raul Santana</h1></span>
					<span class="b"><p>Consectetur adipiscing elit. Praesent neque orci, consectetur id faucibus eget, ullamcorper ut enim. Aliquam sed leo mattis tortor vulputate sagittis. Aenean nisl velit, fermentum non faucibus nec, imperdiet hendrerit enim. Nam non diam ligula. Aenean quis dignissim sem.</p></span>
					<span class="b p5"><!--  --></span>
					<span class="b">
							<span class="b">
								<span class="b"><h1>Quantidade</h1></span>
								<span class="b p5"><!--  --></span>
								<span class="b">
							<ul class="inline tamanChoose">
								<li>Tamanho Escolhido:</li>
								<li><a href="#">PP</a></li>
								<li>Quantidade/Un.:</li>
								<li><select><option>01</option></select></li>
							</ul>
							</span>
						</span>
					</span>
				</span>
				<div class="c"><!--  --></div>
					<span class="b p5"><!--  --></span>
					<span class="b">
						
						<span class="b r p5"><!--  --></span>
						<span class="b r"><a href="#"><span class="b removerProduto">Remover Produto &#9654;</span></a></span>
						
						<div class="c"><!--  --></div>
					</span>
			</div>
		</div>
		<div class="b wrapper contentBottom">
			<img src="img/content-bottom.gif">
		</div>
		</div>
		
<div class="b">
		<div class="b contentBox wrapper">
			<div class="b p checkoutRow">
				<div class="c"><!--  --></div>
					<span class="b p5"><!--  --></span>
					<span class="b">
						<span class="b r"><a href="?page=enderecos"><span class="b finalizarPedido">Finalizar Pedido &#9654;</span></a></span>
						<span class="b r p5"><!--  --></span>
						<span class="b r"><a href="?page=catalogo"><span class="b blueBtn">Comprar Mais &#9654;</span></a></span>
						<div class="c"><!--  --></div>
					</span>
			</div>

		</div>
		<div class="b wrapper contentBottom">
			<img src="img/content-bottom.gif">
		</div>
		</div>
    </div>
</div>